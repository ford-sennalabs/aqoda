...
class Hotel {
  constructor(props) {
    ...
    this.rooms = Array.from({ length: roomsAmout }).map((_, i) => {
      ...
      const roomNumber =
        (i + 1) % this.roomPerFloors === 0
          ? this.roomPerFloors
          : (i + 1) % this.roomPerFloors;
  ...
  findKeyCard(cardNo) {
    const selectedKeyCardIndex = this.keycards.findIndex(
      (keyCard) => keyCard.keyCardNo === cardNo
    );
  ...
  listGuest() {
    ...
    console.log(guessNames.join(", "));
  }

  listGuestInRoom(roomNo) {
    const guessNames = this.findRoom(roomNo);

    console.log(guessNames && guessNames.guess.name);
  }
  ...
  findAvailableRoom() {
    const findRoomAvailable = this.rooms
      .filter((room) => room.available)
      .map((room) => room.roomNo);
    console.log(`${findRoomAvailable}`);
  }
  ...
  roomCheckOut(cardId, guessName) {
    ...
    if (guessName === room.guess.name) {
      ...
      console.log(
        `Only ${room.guess.name} can checkout with keycard number ${room.keycardNo}.`
      );
    }
  }
}
